...
for config in "${configs[@]}"; do
  for batch_size in "${batch_sizes[@]}"; do
    sbatch -p ls6 --gres=gpu:1 --nodelist=gpu8a --wrap="python tools/train.py $config --train_bs=$batch_size --seed=$seed --exp_num=$exp_num --remove_timestamp --exp_suffix=$exp_suffix" -o $log_output
  done
done

for config_2 in "${configs_2[@]}"; do
  for batch_size in "${batch_sizes[@]}"; do
    sbatch -p ls6 --gres=gpu:rtx3090:1 --wrap="python tools/train.py $config_2 --train_bs=$batch_size --seed=$seed --exp_num=$exp_num --remove_timestamp --exp_suffix=$exp_suffix" -o $log_output
  done
done
