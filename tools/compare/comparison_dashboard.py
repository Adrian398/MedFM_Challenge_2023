...
def get_values_from_data(data, exp, setting, metric_keys):
    """Fetch the metric values from the provided data for multiple potential metric keys."""
...
def compare_results(team1, team2):
    ...
    for exp in exps:
        ...
        for setting in settings:
            ...
            metric_keys = task2metric[next(task for task in task2metric if task in setting)]

            for metric_key in metric_keys:
                team1_val = team1_data['task'][exp][setting].get(metric_key)
                team2_val = team2_data['task'][exp][setting].get(metric_key)
                if team1_val is not None and team2_val is not None:
                    comparison_result[exp][setting][metric_key] = {team1_name: team1_val, team2_name: team2_val}
...
for idx, setting in enumerate(settings):
    ...
    source_values = [get_values_from_data(source_data, exp, setting, metric_keys) for exp in exps]
    target_values = [get_values_from_data(target_data, exp, setting, metric_keys) for exp in exps]
...
yaxis_titles = {
    f'yaxis{idx if idx > 1 else ""}_title': task2metric[task][0]
